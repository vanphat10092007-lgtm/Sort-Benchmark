...
- **Custom Comparator** — Tiêu chí so sánh được xây dựng theo yêu cầu của đề bài. Chuỗi có độ dài nhỏ hơn sẽ đứng trước. Nếu hai chuỗi có cùng độ dài thì chuỗi có thứ tự từ điển nhỏ hơn sẽ đứng trước.
...
#### Lý do lựa chọn: Tối ưu tiếp tục so với lần 1 ?
...
So với việc di chuyển trực tiếp các đối tượng string, việc chỉ di chuyển các giá trị nguyên giúp giảm lượng dữ liệu phải xử lý trong mỗi bước Merge.
Ngoài ra, việc tái sử dụng bộ nhớ và tối ưu quá trình so sánh giúp cải thiện hiệu năng thực tế của chương trình trên các bộ dữ liệu lớn.
